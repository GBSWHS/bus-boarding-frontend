...
import { BreadcrumbGroup, ContentLayout, SpaceBetween, Form, Button, Header, Container, FormField, Select, Input, Cards, TableProps, TokenGroup } from "@cloudscape-design/components"
...
function BusEdit() {
  ...
  return (
    <>
      <CustomAppLayout
        contentType="table"
        breadcrumbs={<BreadcrumbGroup items={[{ text: "홈", href: "/admin" }, { text: "버스 목록", href: '/admin/bus'}, { text: '버스 수정', href: window.location.pathname }]} />}
        content={
          <ContentLayout
            header={
              <Header
                ...
                <SpaceBetween size="l">
                  ...
                  <Container header={<Header variant="h2">관리인원</Header>}>
                    <SpaceBetween size='l'>
                      ...
                      >
                        <TokenGroup
                          items={selectedItems.map(item => ({ id: item.id, label: item.studentId + '/' + item.name, dismissLabel: item.name + ' 삭제' }))}
                          onDismiss={(item) => {
                            const newSelectedItems = [...selectedItems];
                            newSelectedItems.splice(item.detail.itemIndex, 1);
                    ...
                    <SpaceBetween size='l'>
                      ...
                      >
                        <SpaceBetween size='xs' direction="horizontal">
                          ...
                          />
                          <Button variant="primary">아래에 추가</Button>
                          <Button variant="normal" href="/admin/station/create" target="__blank">새로운 정류장 생성</Button>
                        </SpaceBetween>
                      </FormField>
                      <FormField
                        label="추가된 정류장"
